...
export class MotoristaRepositoryService implements IMotoristaRepository {
  ...
  get(id?: number): Observable<DriverEntity> {
    if (id) {
      return this.http.get<DriverEntity>("/api/motoristas").pipe(
        map((item) => {
          if (item[0]) {
      ...
      return this.http
        .get<DriverEntity[]>("/api/motoristas")
  ...
  insert(param: DriverEntity): Observable<DriverEntity> {
    return this.http
      .post<DriverEntity>("/api/motoristas", param)
      .pipe(map(this.mapper.mapFrom));
  }
  update(param: DriverEntity): Observable<DriverEntity> {
    return this.http
      .put<DriverEntity>("/api/motoristas/" + param.id, param)
      .pipe(map(this.mapper.mapFrom));
  }
  disableEnable(id: number, status: boolean): Observable<DriverEntity> {
    return this.http
      .patch<DriverEntity>("/api/motoristas/" + id, {
        enable: status,
      })
      .pipe(map(this.mapper.mapFrom));
  }
