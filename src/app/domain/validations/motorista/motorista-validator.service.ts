...
export class MotoristaValidatorService implements IMotoristaValidator {
  ...
  validateRules = (validator: IValidator<DriverEntity>): ValidationResult => {
    return validator
      ...
      .NotEmpty(
        (m) => m.birth_date.toString(),
        this.validatorMessage.required("Data de Nascimento").value,
      )
      .If(
        (m) => m.documents != null && m.documents.length > 0,
        (v) =>
          v.ForEach((m) => m.documents, this.validateDocumentsRules).ToResult(),
      )
      .ToResult();
  };

  validateDocumentsRules = (
    validator: IValidator<DocumentsEntity>,
  ): ValidationResult => {
    return validator
      .NotEmpty(
        (m) => m.doc_type,
        this.validatorMessage.required("Tipo de documento").value,
      )
      .If(
        (m) => m.doc_type !== "",
        (v: IValidator<DocumentsEntity>): ValidationResult => {
          return v
            .NotEmpty(
              (m) => m.number,
              this.validatorMessage.required("Número de documento").value,
            )
            .If(
              (m) => m.doc_type === "CNH",
              (c) =>
                c
                  .NotEmpty(
                    (m) => m.category,
                    this.validatorMessage.required("Categoria da CNH").value,
                  )
                  .ToResult(),
            )
            .ToResult();
